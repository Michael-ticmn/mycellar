...
- [ ] [owner] Apply migration [`supabase/migrations/0011_share_search_path_hardening.sql`](supabase/migrations/0011_share_search_path_hardening.sql) in the Supabase SQL Editor (re-runs the five share-link security-definer functions with locked-down `search_path`, schema-qualified table refs, and a new per-link 1-req-per-2-second rate guard inside `cellar27_share_create_pairing_request`).
